...
# Introspect MoneyConstraintInput and BillingAddressInput
introspect_types = {
    'query': '''query IntrospectMoneyAndAddress {
      money: __type(name: "MoneyConstraintInput") {
        name
        inputFields { name type { name kind ofType { name kind } } }
      }
      address: __type(name: "BillingAddressInput") {
        name
        inputFields { name type { name kind ofType { name kind } } }
      }
    }'''
}
r_t = s.post('https://wiredministries.com/checkouts/unstable/graphql', headers=headers4, json=introspect_types)
print(json.dumps(r_t.json(), indent=2))
