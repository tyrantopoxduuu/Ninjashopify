...
async def test_full_flow():
    ...
    async with httpx.AsyncClient(follow_redirects=True, verify=False, timeout=30.0) as session:
        ...
        # Submit order with the FatZebra token
        place_url = f"https://www.isubscribe.co.uk/ssl/checkout/index.cfm?view=new&step=confirm&mode=admin&action=placeOrder&source=confirm&token={token}&verification={v}"
        r_place = await session.get(place_url, headers=head5)
        print("Place Order URL:", r_place.url)
        print("Place Order Status:", r_place.status_code)
        print("Place Order Text snippet:", r_place.text[:600])
