...
# 4. Try submitForCompletion mutation
headers4 = {
    'accept': 'application/json',
    'content-type': 'application/json',
    'origin': 'https://wiredministries.com',
...
mutation_test = {
    'query': 'mutation Submit($sessionToken: String!) { submitForCompletion(sessionInput: { sessionToken: $sessionToken }) { ...on Receipt { id } } }',
    'variables': {'sessionToken': session_token}
}
r_sub = s.post('https://wiredministries.com/checkouts/unstable/graphql', headers=headers4, json=mutation_test)
print("submitForCompletion Status:", r_sub.status_code)
print("submitForCompletion Response:", r_sub.text)
