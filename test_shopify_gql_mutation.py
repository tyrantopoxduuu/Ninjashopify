...
checkout_session_id = get_meta("serialized-checkoutSessionIdentifier")
...
# 4. Negotiate / Submit Payment Mutation via GraphQL
headers4 = {
    'accept': 'application/json',
    'content-type': 'application/json',
    'origin': 'https://wiredministries.com',
...
# First let's query the Checkout Purchase Order to see what fields/mutations are available
gql_query = {
    'query': 'query GetCheckoutSession($sessionToken: String!) { session(sessionInput: { sessionToken: $sessionToken }) { ...on CheckoutSession { id totalAmountToPay { amount currencyCode } } } }',
    'variables': {'sessionToken': session_token}
}
r_gql = s.post('https://wiredministries.com/checkouts/unstable/graphql', headers=headers4, json=gql_query)
print("GraphQL GetCheckoutSession Status:", r_gql.status_code)
print("GraphQL Response:", r_gql.text)
