...
# 4. Introspect Schema mutations on unstable/graphql
headers4 = {
    'accept': 'application/json',
    'content-type': 'application/json',
    'origin': 'https://wiredministries.com',
...
r_schema = s.post('https://wiredministries.com/checkouts/unstable/graphql', headers=headers4, json=introspect_query)
print("Available Mutations on Shopify unstable/graphql:")
fields = r_schema.json().get('data', {}).get('__schema', {}).get('mutationType', {}).get('fields', [])
for f in fields:
    print(f"  • {f.get('name')}")
